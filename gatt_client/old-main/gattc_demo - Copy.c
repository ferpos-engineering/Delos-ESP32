...
#define GATTC_TAG "DPPS_DONGLE"
#define REMOTE_SERVICE_UUID        0x1815
#define REMOTE_NOTIFY_CHAR_UUID    0xFF01

// STREAM characteristic UUID (128-bit) for 16-byte notifications
// static const uint8_t REMOTE_STREAM_CHAR_UUID128[ESP_UUID_LEN_128] = {
...
static char remote_device_name[ESP_BLE_ADV_NAME_LEN_MAX] = "DPPS-DTS";
...
static void rssi_task(void *arg)
{
    while (1) {
        if (rssi_enabled) {
            esp_err_t err = esp_ble_gap_read_rssi(gl_profile_tab[PROFILE_A_APP_ID].remote_bda);
            
            if (err != ESP_OK) {
                ESP_LOGW(GATTC_TAG, "esp_ble_gap_read_rssi err-code: %d - %s", err, esp_err_to_name(err));
            }

            err = esp_ble_gap_read_phy(gl_profile_tab[PROFILE_A_APP_ID].remote_bda);
            if (err != ESP_OK) {
                ESP_LOGW(GATTC_TAG, "esp_ble_gap_read_phy err-code: %d - %s", err, esp_err_to_name(err));
            }
        }
...
static void ble_rf_tune_on_connect(esp_bd_addr_t peer_bda)
{
    ...
    if (err != ESP_OK) {
        ESP_LOGW(GATTC_TAG, "esp_ble_gap_read_phy failed: %d - %s", err, esp_err_to_name(err));
    }
}
...
static void gattc_profile_event_handler(esp_gattc_cb_event_t event, esp_gatt_if_t gattc_if, esp_ble_gattc_cb_param_t *param)
{
    ...
    case ESP_GATTC_OPEN_EVT:
        ...
        ble_rf_tune_on_connect(gl_profile_tab[PROFILE_A_APP_ID].remote_bda);
        rssi_enabled = true;
        break;
...
case ESP_GATTC_NOTIFY_EVT:
    if (p_data->notify.is_notify){
        ESP_LOGI(GATTC_TAG, "Notification received (len=%d)", p_data->notify.value_len);
    }else{
        ESP_LOGI(GATTC_TAG, "Indication received (len=%d)", p_data->notify.value_len);
    }

    if (p_data->notify.value_len == 16) {
        // 16 bytes stream payload
        ESP_LOG_BUFFER_HEX(GATTC_TAG, p_data->notify.value, 16);
    } else {
        ESP_LOGW(GATTC_TAG, "Unexpected notify length=%d", p_data->notify.value_len);
...
static void esp_gap_cb(esp_gap_ble_cb_event_t event, esp_ble_gap_cb_param_t *param)
{
    ...
    case ESP_GAP_BLE_READ_RSSI_COMPLETE_EVT:
        ESP_LOGI(GATTC_TAG, "RSSI read complete: status=%d, rssi=%d dBm, remote "ESP_BD_ADDR_STR,
                param->read_rssi_cmpl.status,
                param->read_rssi_cmpl.rssi,
                ESP_BD_ADDR_HEX(param->read_rssi_cmpl.remote_addr));
        break;
    case ESP_GAP_BLE_READ_PHY_COMPLETE_EVT:
        ESP_LOGI(GATTC_TAG, "PHY read complete: status=%d, tx=%d, rx=%d",
            param->read_phy.status,
            param->read_phy.tx_phy,
            param->read_phy.rx_phy);
        break;
    case ESP_GAP_BLE_PHY_UPDATE_COMPLETE_EVT:
        ESP_LOGI(GATTC_TAG, "PHY update complete: status=%d, tx=%d, rx=%d",
    ...
    case ESP_GAP_BLE_SCAN_RESULT_EVT: {
        ...
        case ESP_GAP_SEARCH_INQ_RES_EVT:
            ...
            {
                ESP_LOGI(GATTC_TAG, "Scan result, device "ESP_BD_ADDR_STR", name len %u, rssi %d dBm", ESP_BD_ADDR_HEX(scan_result->scan_rst.bda), adv_name_len, scan_result->scan_rst.rssi);
            ...
            if (adv_name != NULL) {
                if (strlen(remote_device_name) == adv_name_len && strncmp((char *)adv_name, remote_device_name, adv_name_len) == 0) {
                    ...
                    if (connect == false) {
                        ...
                        creat_conn_params.phy_mask = 0;
                        // creat_conn_params.phy_mask = ESP_BLE_GAP_PHY_1M_PREF_MASK | ESP_BLE_GAP_PHY_CODED_PREF_MASK;
