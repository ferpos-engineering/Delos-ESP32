...
// Human-readable name for STREAM characteristic (shown via 0x2901 User Description)
static const char stream_char_name[] = "Stream 16B / 10ms";
...
static void auto_io_gatts_profile_event_handler(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param)
{
    ...
    case ESP_GATTS_ADD_CHAR_EVT:
        ...
        if (param->add_char.char_uuid.len == ESP_UUID_LEN_128 &&
            ...
            // Add User Description (0x2901) so apps can show a friendly name
            ...
            esp_attr_value_t user_desc_val = {
                .attr_max_len = sizeof(stream_char_name),
                .attr_len     = sizeof(stream_char_name) - 1, // without '\0'
                .attr_value   = (uint8_t *)stream_char_name,
            };
            
            esp_err_t uret = esp_ble_gatts_add_char_descr(
                gl_profile_tab[AUTO_IO_PROFILE_APP_ID].service_handle,
                &user_desc_uuid,
                ESP_GATT_PERM_READ,
                &user_desc_val,
            ...
            if (uret) {
                ESP_LOGE(GATTS_TAG, "add STREAM user desc failed, error code = %x", uret);
            }
        } else { 
